...
What I'd change: this round of work made failures catchable, not rare, and I'd point the next round at the models that actually reason and write, not another verification layer. gpt-4o-mini runs every step today, tool-choice, retry reformulation, judge verdicts, and answer generation, all off the same constant, and the ambiguous_phrasing category (40% behavior_match, the model confidently picking one interpretation of a multi-grade question instead of asking) looks like an agent-reasoning gap as much as a generation gap, so I'd try a stronger model for the agent's own decisions before assuming a bigger model only helps the final answer. Underneath that, I did build real calculation tools for the multi-step arithmetic table_lookup was failing on. eval-025 was the motivating case for this session's fix: the model called a calculation tool whose formula doesn't cover Junior's threshold-based daily-overs rule, the tool computed outside its own scope, and the answer was wrong. I added a declarative coverage map scoping each tool to the grade its formula actually applies to, so a tool now refuses instead of computing when it's called outside its coverage. Junior's own threshold-based daily-overs formula is deliberately left uncovered by any tool this round, since the fix's job was the coverage boundary itself, not a fourth arithmetic shape: eval-025 still free-texts that calculation and still gets it wrong, flagged in 6 of 7 sampled runs with one delivered clean, and the next round's calculation-tool work should add a tool for that third distinct shape, scoped to junior only, the same way the other two are scoped today. A separate gap stands on its own: eval-018 shows the agent doesn't always defer to its own tool's output even when that output is correct. The agent received its own tool's correct refusal and overrode it in free text in 2 of 5 runs, producing a wrong answer anyway. Every instance was caught by the judge and citation layers and flagged for human review, never delivered silently, but fixing it, making the agent trust its own tool's answer or refusal over re-deriving one in prose, is next round's work, not this one's. The hybrid retrieval experiment's actual failure mode isn't a weighting problem: websearch_to_tsquery ANDs all terms, so a full sentence-shaped tool query returns zero full-text matches and RRF silently falls back to pure dense, which means down-weighting the full-text leg would be a no-op over what's actually running today. The honest next retrieval step is changing what the full-text leg is fed, a short keyword reformulation of the tool query, or filtering the full-text index to playing-conditions sections, before any weighting experiment would mean anything. Dense-only remains the shipped path. Three things are also still interim by design and would need to change before this runs for real users: the static API key swapped for actual Supabase JWT verification, an admin ingestion path so a new association's documents don't require me running a script by hand, and the CI eval gate actually wired up instead of just designed.
...
| Agent deference enforcement (trust a calculation tool's own refusal over free-text re-reasoning, per eval-018) | Every observed instance has been caught by the judge/citation safety net and flagged, not delivered; a real fix needs its own design pass, not a quick patch | ~2-3 days |
...
| Ligature-decoding fix in PDF extraction (ingestion/parse.py, mis-decodes "ti" as "Ɵ") | Found late via a frontend display bug; fixing at the source needs re-ingestion to re-embed and re-store affected chunks, not just a code patch | ~half day plus reingestion time |
